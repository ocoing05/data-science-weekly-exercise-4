...
data %>%
  ggplot() +
  geom_map(map = states_map,
           aes(map_id = str_to_lower(state), fill = cases)) +
  expand_limits(x = states_map$long, y = states_map$lat) +
  theme_map() +
  labs(title="COVID19 Cumulative Cases")
```
  
  One problem this map is that the most populous states are naturally going to have a much higher number of cumulative cases, so this map doesn't really tell us anything about covid in the states.
...
data %>%
  ...
  expand_limits(x = states_map$long, y = states_map$lat) +
  theme_map() +
  labs(title="COVID19 Cumulative Cases Per 10,000 People")
...
(mpls_suspicious <- MplsStops %>%
  ...
  summarize(num_stops = n(), 
            num_sus_stops = sum(ifelse(problem=="suspicious", 1, 0)), 
            percent_sus = num_sus_stops / num_stops) %>%
...
```{r}
pal2 <- colorNumeric("viridis", domain = mpls_all$percent_sus)
leaflet(mpls_all) %>%
  addTiles() %>%
  addPolygons(fillColor = ~pal2(percent_sus), 
              fillOpacity = 0.8,
              color = "#000000", 
              weight = 2, 
              opacity = 1,
  ...
  addLegend(pal = pal2,
            values = ~percent_sus,
            title = "Percent of Suspicious Stops")
```
  
  
  18. Use `leaflet` to create a map of your own choosing. Come up with a question you want to try to answer and use the map to help answer that question. Describe what your map shows. 
  
  Is there a correlation between higher levels of poverty and higher levels of suspicious stops?
  
```{r}
pal3 <- colorNumeric("viridis", domain = mpls_all$poverty)
leaflet(mpls_all) %>%
  addTiles() %>%
...
```
  ...
  My map looks like it might have some similarities with the suspicious percentage map, but it is really hard to tell and draw a definite conclusion. I think it could be informative to compare this map with a map of amount of stops per 10,000 inhabitants, or something along those lines.
  
## GitHub link

  19. Below, provide a link to your GitHub page with this set of Weekly Exercises. Specifically, if the name of the file is 04_exercises.Rmd, provide a link to the 04_exercises.md file, which is the one that will be most readable on GitHub.
  
  [https://github.com/ocoing05/data-science-weekly-exercise-4](https://github.com/ocoing05/data-science-weekly-exercise-4)
  
  [https://github.com/ocoing05/data-science-weekly-exercise-4/blob/main/04_exercises.md](https://github.com/ocoing05/data-science-weekly-exercise-4/blob/main/04_exercises.md)
